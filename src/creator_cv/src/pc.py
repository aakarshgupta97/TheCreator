...
def experiment(pc, bbox, num_clouds=1): 
    y_split = 12 
    x_split = 10 
    x1, y1, x2, y2 = bbox 
    z1 = 0.047 
    ...
    num_grid = np.zeros((10, 12)).astype(np.int32) 
    vox_sizes = [] 
    rad_sizes = [] 
    for i in range(10): 
        for j in range(12): 
            xy_mask = (x_indices == i) & (y_indices == j) & (z_indices >= 0) 
            in_xy = len(pc[xy_mask]) 
            print(in_xy) 
            num_grid[i, j] = in_xy 
            for k in range(8): 
                part_mask = xy_mask & (z_indices == k) 
                in_part = pc[part_mask] 
                num_points = len(in_part) 

                vox_sizes.append(num_points) 
                
                if num_points > 110 * num_clouds: 
                    grid[i, j] = k + 1 

    np.save('num_grid', num_grid) 
    print(sorted(vox_sizes)) 
    plt.imshow(num_grid) 
    plt.colorbar() 
    plt.show() 
...
def voxelizePyramid(pc, bbox, num_clouds=1): 
 ...
 #    print(pc.shape) 
    ...
    np.save('num_grid', num_grid) 
    print([x for x in sorted(vox_sizes) if x > 0]) 
    plt.imshow(num_grid) 
    plt.colorbar() 
    plt.show() 
...
def clean_up_layer(layer, delete_here): 
    for i in range(9): 
        for j in range(11): 
            if layer[i, j] > 0: 
...
def main(): 
    ...
    t2 = time.time() 
    no_floor = new_points[new_points[:, 2] > 0.015] 
    ...
    voxelizePyramid(filtered, bbox, num_clouds) 
    # voxelize(filtered, bbox, num_clouds) 
    # experiment(filtered, bbox, num_clouds) 
