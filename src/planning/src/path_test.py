...
def inflate_gripper(gripper, lock):
    lock.acquire()
    while lock.locked():
        gripper.inflate()
        rospy.sleep(0.5)
...
def move_arm_to(x,y,z, ox, oy, oz, ow, orient_constraint=None):
...
def main():
    ...
    gripper.inflate = lambda: right_gripper.close()
    gripper.deflate = lambda: right_gripper.open()
    ...
    def pick_and_place_block(dest_x, dest_y, dest_z, block_name):
        ...
        move_arm_to(start_x, start_y, start_z, 0, -1, 0, 0)

        raw_input("Press <Enter> to move the arm to pick up the block: ")
        move_arm_to(start_x, start_y, -0.07, 0, -1, 0, 0, orien_const)
        ...
        move_arm_to(dest_x, dest_y, 0.1, 0, -1, 0, 0)

        raw_input("Press <Enter> to drop the block: ")
        move_arm_to(dest_x, dest_y, 0.02 + dest_z, 0, -1, 0, 0, orien_const) # Add 0.02 to avoid collision. Let gravity do the rest.
